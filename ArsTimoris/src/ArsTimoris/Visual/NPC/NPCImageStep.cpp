...
namespace ArsTimoris::Visual::NPC {
    ...
    void NPCImageStep::Render(SDL_Renderer* a_renderer, SDL_FRect a_rect, TimeData* a_timeData) {
        uint8_t oldAlpha;
        ...
        for (std::function<void(RenderPassData*, TimeData*)> func : modifiers) {
            func(&renderPassData, a_timeData);
        }
